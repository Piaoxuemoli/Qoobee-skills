...
实验报告全流程自动化：首次配置个人信息，本地复用；支持自动模式，以及可执行实验、已有数据、纯写作三类报告路径。

Automate lab reports with reusable local profile data, optional auto mode, and three paths: executable experiments, provided-data reports, and paper-only writeups.
